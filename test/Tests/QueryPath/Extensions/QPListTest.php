...
use \QueryPath\Extension\QPList as QPList;

class QPListTests extends PHPUnit_Framework_TestCase {
  public function testAppendList() {
    $list = array('one', 'two', 'three');
    $qp = qp(QP::HTML_STUB, 'body')->appendList($list, QPList::UL);
    ...
    $qp = qp(QP::HTML_STUB, 'body')->appendList($list, QPList::DL);
  ...
  public function testAppendTable() {
    ...
    $qp = qp(QP::HTML_STUB, 'body')->appendTable($data);
    ...
    $o->setRows($data['rows']);
    $qp = qp(QP::HTML_STUB, 'body')->appendTable($o);
    $this->assertEquals(3, $qp->top()->find('th')->size());
    $this->assertEquals(11, $qp->top()->find('td')->size());
    $this->assertEquals('Zwei', $qp->eq(4)->text());
  }
